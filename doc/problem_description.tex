...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The problem can be extended to two dimensions by considering the domain $\Omega=[0,1]\times[0,1]$ and seeking $u(x,y)$:
\begin{equation}
    -\frac{\partial}{\partial x}\left(\exp[\alpha(x,y)]\frac{\partial u}{\partial x}(x,y)\right)-\frac{\partial}{\partial y}\left(\exp[\alpha(x,y)]\frac{\partial u}{\partial y}(x,y)\right) = f(x,y)\qquad\text{for all $0<x<1,0<y<1$}
    \label{eqn:PDE_2d}
\end{equation}
subject to the boundary conditions 
\begin{equation}
    \begin{aligned}
    u(x=0,y) &=0 \qquad{\text{(homogeneous Dirichlet)}}\\
    \frac{\partial u}{\partial x}(x=1,y) &=-g_{\text{right}}(y)\exp[-\alpha(x=1,y)]\\
    \frac{\partial u}{\partial x}(x,y=0) &=g_{\text{bottom}}(x)\exp[-\alpha(x,y=0)] \qquad{\text{(von Neumann)}}\\
    \frac{\partial u}{\partial x}(x,y=1) &=-g_{\text{top}}(x)\exp[-\alpha(x,y=1)]
    \end{aligned}
